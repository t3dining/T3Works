...
つかいかた
    ...
    1日に2回（10:00と17:00）出します。起こすのは2通りあります。
      ・Google（gas/通知.gs の startStoryPost）… 10:15ごろと17:15ごろ。
        マネージの定休日・臨時休業・臨時営業を CLOSED_TODAY で渡してくる。**こちらが本線**
      ・GitHub の予定（cron）… 11:05/11:35 と 18:05/18:35。**Google の回が来なかったときの控え**。
        実際には何時間も遅れて動く（2026年9月は14時台と21時台）
    どちらも「その時刻までに出ているべき枚数」と「もう出した枚数」を見くらべて、
    足りない分だけ出す。もう出ていれば0枚になるので、二重に出ることはない。

    ★予定の回は、休みを自分で決めない（2026-09-19）。
      臨時休業・臨時営業を知っているのは Google の回だけなので、
      その日に Google の回が出したもの（投稿履歴）に従う → 予定の回の休み()
...
# 予定（cron）の回が、その時間帯の Google の回を待つ分。
# Google は :15 と :45 に起こし、どちらも15分ほど前後する → 60分たてば両方済んでいるはず。
# ★gas/通知.gs の STORY_POST_MINUTE・STORY_POST_MINUTE2 を変えたら、ここも見直すこと
Googleを待つ分 = 60
...
def 予定の回の休み(並び, 履歴, 店舗, 日付):
    """GitHub の予定（cron）で動いた回の、休みの決め方。

    予定の回には CLOSED_TODAY が来ない。営業時間.csv の定休日しか分からないので、
    そのまま決めると、臨時休業の日にふつうの商品を出し、臨時営業の日に
    「本日定休日です」を出してしまう（2026-09-19、マニュアルが見つけた⑥）。
    ★とくに夜：予定の回は実際には21時台に動くので、臨時休業の日の夜に
      「営業・目標2枚・済み1枚」と数えて、ふつうの商品を1枚出していた。

    そこで、**その日に Google の回がもう出したもの**に従う。
      ・「おやすみ」の画像が出ている → 休み（その種類を返す）
      ・ふつうの画像が出ている     → 営業（営業時間.csv の定休日は見ない）
    返すもの: (分かったか, 休みの種類)。まだ何も出ていなければ (False, "")
    """
    きょうの分 = [も for も in 並び
              if 履歴.get(f"{店舗['id']}/{も['ファイル']}") == 日付]
    if not きょうの分:
        return False, ""
    for も in きょうの分:
        if (も.get("区分") or "") == "おやすみ":
            return True, (も.get("しらせ") or "定休日")
    return True, ""


def Googleの回を待つか(いま, 時刻たち, 節):
    """その時間帯の Google の回が、まだ来ていないかもしれない時刻か。"""
    if not 時刻たち:
        return False
    時, 分 = (int(x) for x in 時刻たち[min(max(節, 0), len(時刻たち) - 1)].split(":"))
    h, m = (int(x) for x in いま.split(":"))
    return h * 60 + m < 時 * 60 + 分 + Googleを待つ分
...
def main():
    ...
    # ★GitHub の予定（cron）で動いた回か。daily-post.yml が github.event_name を渡してくる。
    #   CLOSED_TODAY が空かどうかでは見分けられない（Google もふつうの日は空を渡すため）。
    #   手で動かした回（workflow_dispatch）と手元の試しは、いままでどおり
    予定の回 = os.environ.get("STARTED_BY", "").strip() == "schedule"
    ...
    for 店舗 in 設定.get("店舗", []):
        ...
        if 予定の回:
            # 予定の回は休みを自分で決めない。Google の回がきょう出したものに従う
            分かった, 休み種類 = 予定の回の休み(並び, 履歴, 店舗, 日付)
            if 分かった:
                print(f"  （予定の回）きょう出したものに合わせます … {休み種類 or '営業'}")
            elif 目標 > 0 and Googleの回を待つか(いま, 時刻たち, 節):
                print(f"  （予定の回）{時刻たち[節]} の Google の回を待ちます"
                      f"（臨時休業・臨時営業を知っているのは Google の回だけです）")
                待った = True
                continue
            else:
                休み種類 = きょうの休み(店舗, 日付, 店舗情報, {})
                if 目標 > 0:
                    # 黙って決めない。この回だけは臨時休業・臨時営業が分からない
                    print("  ★（予定の回）Google の回が来ていません。"
                          "営業時間.csv の定休日だけで決めます（臨時休業・臨時営業は分かりません）")
        else:
    ...
    if 待った:
        # Google の回より先に動いただけ。あとの予定の回でまた見る。失敗にしない
        print(f"\n{日付} は Google の回を待っています")
        return
