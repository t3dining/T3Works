...
function renderShiftCodes() {
  ...
  people.forEach((p) => {
    ...
    //   2つまでは名前、それより多ければ「他◯」にして、全部は title に入れます
...
function saveShiftMemoTags() {
  ...
  //   そこで保存を押すと、**他の端末に届いている登録を空で上書きします。**
 ...
 *    （同じ画面の他の欄に、保存していない分が残っていることがあるので、安全な方に倒します）
...
function 古い画面の帯(出す) {
  ...
  if (!b) {
    ...
    b.innerHTML = '<b>別の端末で、設定が変わりました。</b>この画面は古いままです。'
      + '今のまま保存すると、他の端末で直した分を上書きすることがあります。<br>'
 ...
 *    他の端末で直した内容を、この端末の古い中身で上書きしてしまいます。
...
function importJson(file) {
  ...
  reader.onload = async () => {
    const ok = await askConfirm({
      item: file.name,
      // ★「上書き」ではなく、同じ記録だけを置きかえて混ぜます。他の端末には送りません
      message: 'ファイルの中の記録で、この端末の同じ記録を置きかえます（他はそのまま）。'
        + '他の端末には送りません。よろしいですか？',
    });
    if (!ok) return;
    try {
