...
function parseOiden(text) {
  ...
  if (v.gross !== undefined && 売i >= 0) {
    const あってる = v.net !== undefined && v.tax !== undefined
      && v.net + v.tax === v.gross;
    if (!あってる) {
      const 頭の金 = 下の金(売i, ['消費税総額', '売上内訳', '支払方法']);
      const 組 = [];
  ...
  if (支払.length > 0 && v.gross !== undefined && 支払合計 === v.gross) {
    ['cash', 'credit', 'emoney', 'voucher1', 'kake'].forEach((k) => {
  ...
  const 売上のない日 = 支払.length === 0 && v.net === undefined
    && 税内 === null && 下まで写っている;
  if (売上のない日) {
    ['gross', 'net', 'tax', 'cash', 'credit', 'emoney', 'voucher1', 'kake', 'guests']
      .forEach((k) => { v[k] = 0; });
  }
