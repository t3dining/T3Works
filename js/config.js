 ...
 */
const NIPPOU_GAS_VERSION = '68fef1db';
 ...
 *
 *  ★★ここの金額は、マネージの「年間の売上目標」へ移します（2026年9月5日）。
 *    `js/config.js` は GitHub Pages で誰でも読めるので、会社の売上目標が
 *    店舗別に公開されていました。読むのは `SalesTargets`（設定）です。
 *
 *    下の数字は**移し終わるまでの控え**です。マネージで登録して、円グラフが
 *    今までどおり出るのを確かめたら、**空にします**（`{}` にして、
 *    ここには二度と金額を書き戻さないでください）。
 * ---------------------------------------------------------- */
const SALES_TARGETS = {
 ...
 *   205,000,000＋115,000,000＋88,000,000＋58,000,000＋84,000,000 ＝ 550,000,000 と
 *   ぴったり一致するので、持たせると二重管理になります。
