 ...
 * 現金の次に来る、他の支払い方法
 *
 * ★紙によっては「現金」「7件」「￥3,940」が別々の行に分かれて出てきます。
 *   そこで、現金の行から下へ少し探しに行きます。
 *   ただし、他の支払い方法に当たったらそこで止めます
...
/** 文字を数にする（「3,940」「3.940」→ 3940。数でなければ null） */
...
function cashStackedOf(lines, at) {
  ...
  // 現金のすぐ上が、他の支払い方法の名前なら、並びの先頭ではありません
...
function journalCandidates(lines, field, pairs, seq) {
  for (let i = 0; i < lines.length; i++) {
    ...
    //     掛売 / 0件 / お預かり現金 / ¥0 / ¥71,240 / おつり / ¥18,630
    //   （¥0 は掛売の金額で、お預かりは その次）
...
/**
 * 足りないものを、他の数から埋めます（当てずっぽうではなく引き算です）
 *
...
function journalFill(v) {
  ...
  // ★埋める前に、ありえない値でないか見ます。
  //   歯止めが無かったせいで「現金 −134,760／商品券 243,870」でも
 ...
 *   実際に 現金 −134,760／商品券 243,870 が選ばれました。
 */
 ...
 *
 * ★2026年9月14日の紙は、**組数がその上に乗って**名前7つ・値7つでした。
 *
 *     組数 / 客数 / 男性 / 女性 / 選択なし / 客単価(税込) / 客単価(税抜)
 *     ◯◯組 / ◯◯ / ◯◯ / ◯◯ / 0 / ¥◯,◯◯◯ / ¥◯,◯◯◯
 *
 ...
 *    引くと二重に引いてしまいます。
 *
 *  ★名前と金額が別の行に分かれます（名前だけの行 → 次の行に「1点 3,260円」）。
...
/**
 * 精算レポートの行から、金額を取り出します
 *
 * ★実物のOCRでは「3,260円」が **「3.260m」** になっていました
 *   （カンマが点に、円が m に）。¥ や 円 が付いている前提では読めません。
 * ★「1点」「0点」は件数なので、金額として拾いません。
 */
function seisanMoneyOf(line) {
  const s = cashNormalize(line).replace(/[（）]/g, '').trim();
  if (!s) return null;
  if (/^[(]?\s*\d+\s*点[)]?$/.test(s)) return null;
  const 見つけ = [];
  const re = /(\d[\d,.\s]*)\s*([点組人円¥m]?)/g;
  let m;
  while ((m = re.exec(s)) !== null) {
    const n = Number(String(m[1]).replace(/[,.\s]/g, ''));
    if (Number.isFinite(n)) 見つけ.push({ n, 単位: m[2] });
  }
  if (見つけ.length) {
    const 金 = 見つけ.filter((x) => x.単位 === '円' || x.単位 === '¥' || x.単位 === 'm');
    if (金.length) return 金[金.length - 1].n;
  }
  /* ★「円」が落ちた紙があります（2026年8月4日の2枚目の読み取り。
       281,940 / 90,240 / 17,320 … と、ほとんどの円が消えていました）。
     ★そのときは「**行がまるごと数字**」のものだけを金額とみなします。
 ...
 *       [クレジット]      ← 本当は [クレジット明細]
 *
 *   「明細」の字だけで探すと、**2か所が同時に壊れます。**
 *     ・支払内訳がどこで終わるか（`seisanPayRows`）… 明細の行まで読みこんでしまう
 *     ・明細がどこから始まるか（`parseSeisan`）  … その他支払明細まで飛び越す
 *   実物の8月8日では2つ目だけが表に出ましたが、**支払内訳が積み上がる形の紙**では
 *   1つ目も壊れます（本物の紙42枚の見出しを崩して確かめました・2026-09-16）。
 *
 * ★かぎかっこで始まる見出しも見ます。支払内訳の中の行はかぎかっこが付かない
 *   （ただの「クレジット」）ので、取りちがえません。
 */
function seisan明細の見出し(line) {
  const p = cashPlain(line);
...
/**
 * 支払内訳の4つを読みます（現金・クレジット・その他支払・売掛金）
 *
 * ★名前・件数・金額が別々の行に出ます（現金 / 2点 / 41,205円）。
 * ★「売掛金 / 0点 / 円」のように **0が落ちる**ことがありました。
...
function seisanPayRows(lines, から) {
  /* ★支払内訳は「明細」の見出しの**手前まで**です。
       ...
       2026年8月4日の紙で実際に起きました（クレジット 90,240円 のはずが、
       出前館クレジットの 6,310円 を読んでいました）。 */
  let 終わり = lines.length;
  for (let i = から; i < lines.length; i++) {
    if (seisan明細の見出し(lines[i])) { 終わり = i; break; }
  }
  ...
  /* ---- 積み上がった形：名前が先に全部並び、そのあとに件数と金額 ----
       ...
       点点点点                         ← 崩れた残骸。飛ばします
       281,940円 / 90,240円 / 吧 / 吧   ← 0円は「吧」などに化けて落ちます
  ...
  /* ★「円の付いた金額が1つも無い」ときは、積み上がった形とは**みなしません。**
       円が丸ごと落ちた紙があります（2026年8月4日の2枚目の読み取り。
       281,940 / 90,240 … と、ほとんどの円が消えていました）。
       それを「金額が無い＝積み上がった形」と取って、**ふつうに読めるものまで
       捨てていました。**円が無いときは、ふつうの形として読みます。 */
...
/**
 * 明細（クレジット・その他支払）を読みます
 *
 * ★実物のOCRはこうでした。**見出しが名前と金額のあいだに入りこみます。**
 *
 *     [ク  下明細]          ← 見出しが大破
 *     Uberク沙下            ← 名前
 *     死下,10·QUICPay       ← 名前
 *     [老の他支払明細]      ← ここに割りこむ
 *     1点 / 3.260m / 12,480円 / 電子▽一 / 1点 / 13,920円
 *
 *   節で切ると、名前と金額が別の節に分かれてしまいます。
 *   そこで**節を見ず、支払内訳の下を丸ごと見て**、出てきた順に結びます。
 *
 * ★順番で結ぶのは危ういので、**必ず紙の合計で検算します**。
 *   合わなければ使いません。
 */
function seisanDetailRows(lines, から) {
  const 名前 = [];
  const 金額 = [];
  const 当たる = (line) => {
    const p = cashPlain(line);
    for (const f of SEISAN_CREDIT.concat(SEISAN_OTHER)) {
  ...
  for (let i = から; i < lines.length; i++) {
    ...
    /* ★件数の「点」が落ちて、裸の数になることがあります。
           ...
           Uberク / y下 / 47 / 8,250円     ← 「4点」が「47」に化けた
...
function parseSeisan(text) {
  ...
  // 支払内訳がどこから始まるか。ここから下が「支払の話」です
  let 支払から = -1;
  for (let i = 0; i < lines.length && 支払から < 0; i++) {
    if (cashPlain(lines[i]).includes('支払内訳')) 支払から = i;
  }
  if (支払から < 0) {
    return { v, checks: [], fixed: [], sure: {}, cut: false, ok: false,
      missing: ['支払内訳'], why: '支払内訳の欄が見つかりません' };
  }
  ...
  /* ★「明細」の見出しから探します。
       ...
       **売掛金の名前が支払内訳の上の方にまとまって出る紙**があり、
       支払内訳の金額（41,205 など）を明細の金額として拾っていました（2026-09-10）。
  ...
  for (let i = 支払から + 1; i < lines.length; i++) {
    if (seisan明細の見出し(lines[i])) { 明細から = i + 1; break; }
  }
  if (明細から < 0) {
  ...
  /* ③ 総売上・純売上
     ★OCRで「総売上」の総が落ち、「純売上」が「純亮上」になっていました。
       字が当てにならないので、**売上の欄に出る最初の2つの金額**を使います
       （紙は必ずこの順に出ます）。取りちがえても、下の検算で捕まります。 */
  /* ★オーダの見出しが、金額より**先**に出る紙があります（2026年8月6日）。
  ...
  for (let i = 0; i < 支払から; i++) {
    const p = cashPlain(lines[i]);
    if (/内税|組数|客数|単価|明細/.test(p)) break;
    /* ★見出しが3つ以上積み上がる紙があります（2026年8月28日・こじゃれ）。
       ...
       「オーダ」で打ち切っていたので、**純売上を1つも読めませんでした。**
       金額が2つそろうまでの「オーダ」「値割引」は、**ただの見出し**です。
     ★その行に金額が付いていれば、**本物のオーダ・値割引の行**なので打ち切ります。
       （`オーダ ◯,◯◯◯点 ◯,◯◯◯,◯◯◯円` の売上を純売上にしないため） */
    if (/オーダ|才一夕|値引|割引/.test(p)) {
      if (売上まで.length >= 2 || その行の金(lines[i]).length) break;
      continue;
    }
  ...
  for (let i = 0; i < 支払から; i++) {
    ...
    /* ★「人」が字に化けることがあります（`137A`。2026年8月11日の紙）。
  ...
  /* ★見出しの「客数」の字ごと崩れることがあります（`組数・数`。8月11日の紙の
       2通り目）。そのときは、支払内訳より上に**ただ1つだけある「◯人」**を使います。
     ★2つ以上あったら読みません（どれが客数か決められないため）。
     ★まちがえても「総売上 ÷ 客数 ＝ 客単価」で止まります。 */
  if (v.guests === undefined || v.guests === null) {
    const 人たち = [];
    for (let i = 0; i < 支払から; i++) {
      const 人 = journalUnitOf(lines[i], '人');
      if (人 !== null) 人たち.push(人);
    }
    if (人たち.length === 1) v.guests = 人たち[0];
  }
  ...
  for (let i = 0; i < 支払から; i++) {
    ...
    /* (b) 見出しだけが1行に並ぶ形。2026年8月4日の紙で出ました。
             ...
             組数・客数 組単価 客単価         ← 見出しが横並び
             11組 / 62人 / 26,584円 / 4,378円

             組数客数 組単価 客単価 点単価     ← 同じ紙の、別の読み取り
             11 6 / 62人 / 26,584 / 4,378 / 540円

       ★**客数（◯人）の行のあと**から数えます。見出しの行のすぐ下から
         数えると、「14組」「14 8」まで数に入ってしまい、数が合いません。
       ★残りの数を、見出しの「◯単価」の並び順で結びます。
    ...
    if (!取れた) {
      ...
      // 客数（◯人）の行を探します。見つからなければ見出しの行から
      let から5 = i;
      for (let j = i; j < Math.min(i + 6, 支払から); j++) {
        if (journalUnitOf(lines[j], '人') !== null) { から5 = j; break; }
      }
      const 金 = [];
      for (let j = から5 + 1; j < 支払から; j++) {
  ...
  /* ⑤' 客数の見出しごと崩れた紙
       ...
       数・数 組単価 客単価        ← 「組数・客数」が「数・数」に崩れています
       11 / 62A / 26,584円 / 4,378円 / 単価
            ↑「85人」の人が A に化けています

     2026年8月4日の紙の、3通り目の読み取りです。
     ★見出しで探せず（客数の字が無い）、人でも探せません（Aに化けた）。
     ★そこで**列の数**で数えます。「◯単価」の見出しがN個あるなら、
       それより前にある数が 組数・客数 です。**客数はその最後**です。

         値 = [11, 62, 26,584, 4,378]   見出し = [組単価, 客単価]
         余り = 4 − 2 = 2  → 組数=14、客数=85
         客単価 = 値[2 + 1] = 4,378

     ★**客数がどうしても読めなかったときだけ**通ります。ふつうに読めた紙の
       読み方は変わりません。
     ★余りは2つ（組数・客数）までです。それ以上は読みません。
     ★まちがえても「総売上 ÷ 客数 ＝ 客単価」で止まります。
       実際、組数を客数と取りちがえたら、この検算が合いません。 */
  if (v.guests === undefined || v.guests === null) {
    for (let i = 0; i < 支払から; i++) {
      const p6 = cashPlain(lines[i]);
      if (!p6.includes('客単価')) continue;
      const ラベル = [];
      ['組単価', '客単価', '点単価'].forEach((h) => {
        const at = p6.indexOf(h);
        if (at >= 0) ラベル.push({ h: h, at: at });
      });
      ラベル.sort((a, b) => a.at - b.at);
      const 値 = [];
      for (let j = i + 1; j < 支払から; j++) {
        if (/単価|内税|明細|支払|割引|値引/.test(cashPlain(lines[j]))) break;
        const m = seisanMoneyOf(lines[j]);
      ...
      const 余り = 値.length - ラベル.length;
      const n6 = ラベル.map((x) => x.h).indexOf('客単価');
      if (余り >= 1 && 余り <= 2 && n6 >= 0) {
        v.guests = 値[余り - 1];       // 組数・客数のうち、あとの方が客数
        v.per = 値[余り + n6];
      }
      break;
    }
  ...
  /* ★税額は、その欄に出てくる**最後の金額**です。
       (内税8%対象 / 3,260円) ( / 1点 / 241円)   → 3,260 は対象額、241 が税額
     はじめ「件数の行を通り過ぎてから拾う」としていましたが、
     **値割引の行（0点／吧）が内税の欄に割りこんだ紙**があり、
     対象額の 3,260 を税額として拾ってしまいました（2026-09-10）。
  ...
  /* ★外税（上に乗せる税）が出る紙があります（2026年8月7日）。
         ...
         外税(10%対象 / 3,150円) / 2点 / 380円

     ★内税とちがって、**オーダの合計には入っていません。**
         総売上 715,790 ＝ オーダ 715,410 ＋ 外税 380
       そのため、次の2つが**外税の分だけずれます**。
         純売上   総売上 − 内税 − **外税** ＝ 純売上
         客単価   (総売上 − **外税**) ÷ 客数 ＝ 客単価
  ...
  const fixed = [];
  const 埋めた元 = {};
  /* ★クレジットが読めないときは、明細の合計で埋めます。
       ★ロケットナウを足し忘れていました（2026-09-11に気づきました）。
         忘れると、ロケットナウのある日に**その分だけ足りない**クレジットを作り、
         「現金＋クレジット＋その他支払＋売掛金 ＝ 総売上」が合わなくなります。
         止まるので害はありませんが、読める紙が読めなくなります。
       ★元にした項目は全部覚えます。あとで「その元が確かか」を見るためです。 */
  if (!has('creditAll') && has('uberCard') && has('cardId')) {
    const もと = ['uberCard', 'cardId', 'demaeCard', 'rocket'].filter(has);
    v.creditAll = もと.reduce((a, k) => a + v[k], 0); fixed.push('クレジット');
    埋めた元.creditAll = もと;
  }
  /* ★「電子マネー ← その他支払」という埋め方は**やめました**。
       その他支払明細は1種類ではありません。ホットペッパーだけの日に、
       その370円を電子マネーに入れてしまうところでした（2026-09-10）。
  ...
  /* クレジット明細に出なかったものは、0円の日です。
     ...
     ★入れてよいのは「**残りがちょうど 0 のとき**」だけです。
       残りをそのまま入れると、**他の金額の読みまちがいを吸い込んで**
       合計が合ってしまいます（試験で Uber を 3,260→3,234 に崩したら、
 ...
 *
 *  ★★1店舗も選んでいないときは、**どこにも書きません。**
 *
 *    はじめは「1つも選んでいなければ全店舗」にしていました（URLを入れたら
 *    全部テストへ、という前からの動きに合わせたためです）。
 *    **これが落とし穴でした。**2026年9月11日、バグるでジャーナルを日報に書いたのに
 *    本物の日報に入っていませんでした。**こじゃれ用に入れたURLが効いていて、
 *    バグるの数字がテスト用の日報に入っていました。**
 *    アプリは「書けました」と出します（本当に書けているので）。
 *
 *    ★どちらに倒しても危ない、というのがこの設定の性質です。
 *        全店舗テストに倒す → 本物の日報に入らない（今回のかたち）
 *        全店舗本番に倒す   → 試した数が本物の日報に入る
 *      **だから倒しません。**選ぶまで書かせず、画面で選んでもらいます。
 * ---------------------------------------------------------- */
const NippouTestStores = {
  ...
  /** その店舗は、テスト用の日報に書くか（選んでいなければ false） */
  has(storeId) {
    return this.all().indexOf(storeId) >= 0;
  },
};
 ...
 * テスト用の設定が「中途半端」か
 *
 * ★URLだけ入れて、店舗を1つも選んでいない状態です。
 *   このまま書かせると、**どちらに倒しても事故になります**（上の説明）。
 *   書く手前で止めて、画面で選んでもらいます。
 */
function nippouTestHanpa() {
  return !!NippouTest.get() && !NippouTestStores.all().length;
}
 ...
 */
/* ★2026-09-21 から**どの店舗もここには入りません**（ko-dai さん「おいでんテラスはポイントの読み取りをやめ、
     手入力に変えてください」）。おいでんテラスだけが紙の「ポイント」を日報のリクルートポイントに
     書いていましたが、日報はリクルート・ぐるなび・食べログに分かれていて、
     **紙はその内わけを出しません。**まとめた数を1つの行に入れると、内わけが崩れます。
   ★読むのは今までどおりです（検算に要ります）。画面の表と日報から外れるだけです。 */
const JOURNAL_POINT_STORES = [];
function journalPointOk(storeId) {
  return JOURNAL_POINT_STORES.indexOf(storeId) >= 0;
}

/* ------------------------------------------------------------
 *  紙に出ないので、人が入れて日報に書く行（2026-09-21・ko-dai さん）
 *
 *  ★レジの紙は「ポイント」「商品券（釣無し）」「商品券（釣有り）」とまとめて出します。
 *    日報は種類ごとの行に分かれています（リクルート／ぐるなび／食べログ、
 *    西町／プレミアム／プレミアムデジタル）。**紙からは分けられません。**
 *    そこで、その日にあった分だけ人が入れて、そのまま日報のその行に書きます。
 *  ★**空のままなら、日報には何も書きません**（ko-dai さんの決め）。
 *    0 を書くと、日報に先に入っていた数を消してしまいます。
 *  ★こじゃれ（精算レポート）はここに入りません。紙が最初から内わけを出すので、
 *    読んだ数をそのまま書いています（cashNippouRowsFor）。
 *  ★入れる欄は、表の「手で直す」欄と同じものです。打った数がそのまま日報に入ります。
 * ---------------------------------------------------------- */
const JOURNAL_手で入れる行 = {
  sumimaro: ['recruit', 'gurunavi', 'tabelog', 'ticketA', 'ticketB', 'ticketC'],
  chacoru:  ['recruit', 'gurunavi', 'tabelog', 'ticketA', 'ticketB', 'ticketC'],
  popo:     ['recruit', 'gurunavi', 'tabelog', 'ticketA', 'ticketB', 'ticketC'],
  oiden:    ['recruit', 'gurunavi', 'tabelog', 'ticketA', 'ticketB', 'ticketC'],
  baguru:   ['ticketO'],
};

/** その店舗で、人が入れて日報に書く行（無ければ空） */
function journal手で入れる行(storeId) {
  return JOURNAL_手で入れる行[storeId] || [];
}
...
function oidenPayNameOf(line) {
  const p = cashPlain(line);
  if (!p) return '';
  /* ★★「数字＋件」で切っていましたが、**数字と件のあいだに別の字が入る**紙があります
       （2026年8月12日の紙・組み直した文字。OCRのまちがいです）。

           現金 1 芋 件 ◯◯,◯◯◯ 円      ← 「1」と「件」のあいだに「芋」
           QR 支 払 茎 件 ◯◯,◯◯◯ 円    ← 数字そのものが「茎」に化けた

       切れずに行がまるごと残り、「円」を含むので**名前ではない**と判断され、
       現金とQRの行を見落としました。クレジットの行だけが名前になり、
       **QRの金額までクレジットに入って**「支払方法の合計 ＝ 総売上」で止まりました。
     ★支払方法の名前に**数字は入りません**（現金・クレジットカード支払・QR支払・
       ポイント(ホットペッパーグルメ)）。そこで**最初の数字か「件」のところで**切ります。
       残った化けた字（「QR支払茎」の茎）は、下の見分けに影響しません。 */
  const 名 = p.replace(/[\d件].*$/, '').trim();
  if (!名) return '';
...
function parseOiden(text) {
  ...
  /**
   * その見出しの行と、すぐ下から次の見出しまでの金額を全部
   *
   * ★★**見出しの行に値が載る紙があります。**場所から組み立て直した文字では
   *   **毎回こうなります**（1行にまとめるのが組み直しの仕事だからです）。
   *
   *       売上内訳 ◯◯,◯◯◯円      ← 値が見出しと同じ行
   *       10% 標準                ← こちらが空
   *
   *   見出しの**下だけ**を数えていたので、純売上も消費税内訳も**1つも読めません**でした。
   *   2026年8月20日の紙（組み直した文字）で、売上内訳・消費税内訳・消費税総額・総売上の
   *   **4つが同時に落ちました**（2026-09-16）。
   * ★見出しの行を数に入れるだけです。ふつうの紙の見出しには数字が無いので、
   *   これまでと変わりません。
   */
  const 下の金 = (i, 止め) => {
    const out = [];
    const 見出しの金 = seisanMoneyOf(lines[i]);
    if (見出しの金 !== null) out.push(見出しの金);
  ...
  // ★①で拾った消費税が、内訳と食いちがうことがあります（見出しが崩れた紙）。
  //   下の「内訳から決める」で直します

  /* ★★見出しが崩れて、欄の切れ目が分からなくなる紙があります。

       2026年8月28日の紙では「売上内訳」が **「内」だけ**になり、
       消費税の金額が**見出しより前**に出ていました。
       そのせいで「消費税総額のすぐ下の金額」として**純売上を
       消費税として拾って**いました。検算（消費税内訳の合計 ＝ 消費税総額）が
       食いちがいを捕まえて止まりましたが、読めない日になります。

     ★そこで**足し算で決め直します。**総売上 ＝ 純売上 ＋ 消費税 になる組み合わせを、
       売上の欄に出ている金額の中から探します。
     ★★**両方とも紙に出ている数**でなければ使いません。引き算で作った数は使いません。
       作った数を入れると、**検算が自分で自分を通して**しまいます。
     ★組み合わせが2通り以上あるときも使いません。決め手がないためです。 */
  /* ★★「売上の欄」の金額を、**見出しの位置ではなく、内訳より前**で取ります。

       2026年8月23日の紙では、見出しが3つとも一番上に固まっていました。

           総売上 / 売上 / 消費税総額     ← 見出しだけ先に3つ
           おいでんテラス / 精算 / …
           ◯◯,◯◯◯円                  ← 総売上
           ◯◯個
           ◯◯,◯◯◯円                  ← 純売上
       ...
       「売上」の見出しのすぐ下は次の見出しなので、**金額が1つも取れません**でした。
     ★見出しから数えるのをやめ、**内訳の欄が始まる前にある金額を全部**集めます。 */
  let 内訳のはじめ = lines.length;
  ['売上内訳', '消費税内訳', '支払方法'].forEach((語) => {
    const n = 見出し(語);
    if (n >= 0 && n < 内訳のはじめ) 内訳のはじめ = n;
  });
  const 頭の金 = [];
  for (let n = 0; n < 内訳のはじめ; n++) {
    const x = seisanMoneyOf(lines[n]);
    if (x !== null) 頭の金.push(x);
  }

  /* ★内訳から総売上を決めます。
       純売上（売上内訳の合計）と 消費税（消費税内訳の合計）が両方あるなら、
       総売上はその足し算です。**その数が紙に出ていれば**使います。
     ★出ていなければ使いません。作った数を入れると、検算が自分で自分を通します。 */
  if (v.net !== undefined && 税内 !== null) {
    const たし = v.net + 税内;
    if (頭の金.indexOf(たし) >= 0) { v.gross = たし; v.tax = 税内; }
  }

  if (v.gross !== undefined) {
    const あってる = v.net !== undefined && v.tax !== undefined
      && v.net + v.tax === v.gross;
    if (!あってる) {
      const 組 = [];
      for (let a = 0; a < 頭の金.length; a++) {
        for (let b = 0; b < 頭の金.length; b++) {
          if (a === b) continue;
          const 大 = 頭の金[a];
          const 小 = 頭の金[b];
          if (大 <= 小 || 小 <= 0 || 大 >= v.gross) continue;
          if (大 + 小 === v.gross) 組.push([大, 小]);
        }
      }
      // 同じ組み合わせが2回出ることがあるので、中身で数えます
      const ちがう = 組.filter((x, n) =>
        組.findIndex((y) => y[0] === x[0] && y[1] === x[1]) === n);
      if (ちがう.length === 1) { v.net = ちがう[0][0]; v.tax = ちがう[0][1]; }
    }
  }
  ...
  if (支i >= 0) {
    ...
    /* ★★支払方法が**1つだけ**の日は、ふつうの形でも「名前が金額より先」になります。
         （名前が1つなら、最後の名前＝最初の名前なので必ず金額より前です）
         積み上がりだと取りちがえると、「◯件」の行で区切れず**1つも読めません。**
         2026年8月20日の紙（全額クレジット・組み直した文字）で起きました。
       ★積み上がりは「名前が**2つ以上**、金額より先にならぶ」形です。
         名前が1つなら、ふつうの読み方で正しく読めます。 */
    const 名の数 = 行.filter((x) => x.名).length;
    const 積み上がり = 名の数 >= 2 && 最後の名 >= 0 && 最初の金 >= 0 && 最後の名 < 最初の金;
    if (積み上がり) {
  ...
  /* ★出ていない支払方法は、その日は使われていません。**0円です。**
       ...
       おいでんテラスの紙は、**使わなかった支払方法の行が出ません。**
       全額クレジットの日は、現金の行もQR支払の行もありません。

     ★ただし**支払方法の合計が総売上と合ったときだけ**0にします。
       合わないなら、読めていない支払方法があるということなので、
       0にすると**現金やQRのあった日を0円で記録**してしまいます。
     ★合計は**出ていた分だけ**で計算しています。ここで足す0は合計に入りません。
       埋めた数が自分で自分を通す形（堂々めぐり）にはなっていません。

     ★2026-09-13、はじめは現金だけ0にしていました。そのせいで
       **電子マネーの欄だけ「—」（読めない）と出て**、ko-dai さんに
       「QR支払が読み込めない」と言われました。**同じ理屈なのに、
       片方にしか当てていませんでした。** */
  const 支払合計 = 支払.reduce((a, x) => a + x.金, 0);
  if (支払.length > 0 && v.gross !== undefined && 支払合計 === v.gross) {
    // ★おいでんテラスの紙にポイントの欄はありません。0で埋めておくと、
    //   画面が「—（読めない）」ではなく「0（書きません）」になります
    ['cash', 'credit', 'emoney', 'point', 'voucher1', 'kake'].forEach((k) => {
  ...
  /* ---- ★売上のない日（0円の日） ----
       ...
       おいでんテラスは、**売上が1円も無い日**の紙も出ます。そのとき紙には
       **支払方法の欄そのものが出ません。**売上内訳も消費税内訳も、見出しだけで
       中身がありません。

     ★数字を見て決めません。**欄が出ているかどうか**で決めます。
       実際の紙で、OCRが 0円 を「20円」と読んだ箇所がいくつもありました
       （不支払額・支払額・サービス料・入金…）。**その紙の「0人」も
       「20人」かもしれません。**数字は当てになりません。

     ★★写真が途中で切れた日と、見分けが要ります。
       支払方法の欄が無いのは「その日は無かった」だけでなく
       「そこまで写っていない」でも起きます。**後者で0円と記録したら、
       売上のあった日が0円で残ります。**
       そこで、**支払方法より下にある見出し**（レジ点検・金種・担当者・会計回数）が
       写っていることを見ます。下まで写っているのに支払方法が無いなら、
       **レジが刷らなかった**＝その日は無かった、と言えます。 */
  const 文字 = lines.map((l) => cashPlain(l)).join('\n');
  const 下まで写っている = /レジ点検|金種|担当者|会計回数/.test(文字);
  /* ★★ここまでの3つ（支払方法が無い・純売上が無い・消費税内訳が無い）は、
       どれも**「読めなかった」**です。**「無かった」ではありません。**

     ★2026年8月20日の紙（組み直した文字）で、**売上のある日を「売上のない日」と決めつけ、
       全部0円を返し、しかも検算が3つとも通りました**（0＝0 なので当たり前に通ります）。
       紙には 総売上 ◯◯,◯◯◯円 とはっきり出ています。
       **まちがった数が、確かめられた顔をして日報に入る**形です。一番危ない壊れ方です。
       （このときは、いまの文字と食いちがったので合わせが空欄にして助かりました）

     ★そこで「無かった」の**証し**を紙から取ります。**客数が0人と読めている**ことです。
       0人は紙に印刷されている数で、読めなかったこととは別ものです。
     ★OCRが「0人」を「20人」と読むかもしれません。そのときは**この日は止まります**
       （空欄になるだけで、まちがった数は入りません）。安全な方に外れます。 */
  const 売上のない日 = 支払.length === 0 && v.net === undefined
    && 税内 === null && 下まで写っている && v.guests === 0;
  if (売上のない日) {
    ['gross', 'net', 'tax', 'cash', 'credit', 'emoney', 'point', 'voucher1', 'kake', 'guests']
      .forEach((k) => { v[k] = 0; });
  }
  ...
  /* ★売上のない日は、足し算で確かめるものがありません。
       代わりに**そう判断した理由**を検算の欄に出します。
       画面に出るので、人が見て「本当にそうか」を確かめられます。 */
  if (売上のない日) {
    checks.push({
 ...
 *   oiden  … 精算（おいでんテラス）
 *
 * ★★**6店舗全部書きます。既定値は置きません**（2026-09-13）。
 *
...
function parseCashFor(storeId, text) {
  ...
  /* ★0円でも `read`（読めた）です。`none`（現金の行が見当たらない）にはしません。

       おいでんテラスの紙は、**現金の会計が無かった日は現金の行が出ません。**
       他の店舗なら「行が見当たらない＝読めていないかもしれない」ので
       黄色で「紙に現金の行があるのに0円のときは手で直してください」と出しますが、
       おいでんテラスでは**0円だと確かめられています**
       （支払方法の合計 ＝ 総売上 が通っているので、現金が入る余地がありません）。

     ★確かめられているのに「読み取れなかったかも」と出すと、
       **合っているものを疑わせます。**実際に「読み取れないになっている」と
       言われました（2026-09-13）。読めているなら、読めたと言います。 */
  return { yen: r.v.cash, how: 'read' };
}

/**
 * こじゃれ（精算レポート）の、日報への入れ方
 *
 * ★4店舗とちがい、**引き算をしません。**
 *   4店舗の紙は「クレジット」に出前館やウーバーが混ざっているので引きますが、
 *   こじゃれの紙は**最初から分けて出ている**ので、そのまま入れます。
 *   引くと二重に引いてしまいます。
 *
 *     B3  現金売上        ← 現金
 *     B4  クレジット      ← クレジット・iD・QUICPay（Uber と出前館を除いた分）
 ...
 *     B30 当日客数        ← 客数（人）
 */
const SEISAN_TO_NIPPOU = {
  cash: 'cash',            // 現金        → 現金売上
  cardId: 'credit',        // iD・QUICPay → クレジット
  demaeCard: 'demaeCard',  // 出前館クレジット
  emoney: 'emoney',        // 電子マネー
...
const NIPPOU_LABELS = {
  ...
  /* ★4店舗（日計レポート）の紙に出る「ポイント」は**ホットペッパーグルメ**です。
       日報では「リクルートポイント」の行に書きます（ko-dai さん・2026-09-13）。
     ★こじゃれの `recruit` と**同じ行**に書きますが、鍵は分けてあります。
       こじゃれの紙は「ホットペッパー」と名前で出るのに対し、
       4店舗の紙は「ポイント」としか出ないためです。読む側が別なので、
       1つにまとめると、どちらの紙から来たのか分からなくなります。 */
  point:     'リクルートポイント',
  ...
  // ★バグるだけの商品券（紙には出ません。人が入れます・ko-dai さん 2026-09-21）
  ticketO:   'おむすび商品券',
};
...
const NIPPOU_MINUS = {
  ...
  point:  [],          // ★4店舗のポイント（ホットペッパーグルメ）。引くものはありません
  net:    [],
  guests: [],
};
 ...
 */
const NIPPOU_GAS_VERSION = '6bc29c3a';
...
/* ------------------------------------------------------------
 *  管理用PINが要る「記録の直し方」（op の種類）
 *
 *  ★`ADMIN_SETTINGS` は**設定**の一覧ですが、こちらは**記録に当てる op** です。
 *    `cloudflare/worker.js` の `管理が要る()` と `gas/コード.gs` の `needsAdmin_` が
 *    **同じ種類**を断ります。3か所がずれると、下のどちらかが起きます。
 *      ・サーバーが断るのにアプリが捨て方を知らない → **送信箱が永久に詰まります**
 *      ・アプリが捨てるのにサーバーは通す → 直したのに元に戻ります
 *
 *  ★2026-09-23、`voiceReply` を管理だけにしたとき、**ここへ足し忘れました。**
 *    管理でない人がご意見に返事を書くと、その端末の送信箱が**黙って止まります**
 *    （クローズのチェックも一緒に止まります）。シフトの分野が見つけました。
 *    → `test_op_kinds.py` が3か所を見くらべます。
 * ---------------------------------------------------------- */
const ADMIN_OP_KINDS = ['voiceReply'];

/** op の種類の、画面に出す呼び名 */
const OP_KIND_NAMES = { voiceReply: 'ご意見の返事' };
// ★`shiftStaff`（シフトの名簿）は、2026-09-07 に**わざと外しました。**
//   ko-dai さんの判断で、**各店長の端末から名簿を直せるようにする**ためです。
//   緩むところ（ふつうのPINで名前も番号も書ける）を先に数えて伝えたうえでの決定です。
//   ★**戻すときは、使い方2冊（ワークス6章・マイン2章）の名簿の説明も戻します（マニュアルの分野へ知らせてください）。**
//     外れているあいだ `shiftRosterNeedsAdmin()` は偽で、**赤い警告は出ません**。
//     戻すと警告も戻ります。説明が無いと、店長は赤い帯の意味が分かりません。
//     ★戻す人が**必ず目にするのは、ここのコメントだけ**です（2026-09-24、マニュアルの分野の願い）。
...
const CATCH_ONLY_STORES = [
  ...
  //    出すときは、この行の  off: true,  を消すだけです（他は直しません）。
 ...
 * 「渡した相手」を入れてもらう開始日
 *
 * これより前の分は、人数と金額だけで記録できます（欄そのものを出しません）。
 * 誰に渡したかを控えていなかった月に必須の欄を出しても、手が止まるだけだからです。
 *
 * ★ここを動かせば、画面の出し入れも、保存のときの確かめも、
 *   入力画面に出る「◯月までの分は…」の一文も、全部ついてきます。
 *   月の名前を他の場所に書かないこと（片方だけ古いまま残ります）。
 *
 * 2026-09-16：9月1日から10月1日へ動かしました。
 *   9月は表（支払い金額管理表）でも記録していて、そちらに相手の列が無いため、
 *   アプリだけ必須にすると入れられない人が出ます。
 */
const CATCH_WHO_FROM = '2026-10-01';
 ...
 *     他の4店舗 … 仕込み／営業（→ SHIFT_SLOTS_STORES）
 ...
 *   patty     … パティのボタンを出すか
 *
 * ★時刻は '17' = 17時、'17.5' = 17時半 の書き方です（他と同じ）。
 */
/** 入れ方の設定を入れておく名前 */
...
/* -------- 人が足りないマス --------
 *
 * ★元のスプレッドシートで、赤く塗って「ここにもう1人欲しい」と
 ...
 * ---------------------------------------------------------- */
const SHIFT_REQ_STORE = '_shiftreq';
/**
 * ★申請の画面を出すか（提出ページの「番号をまだ持っていない」と、名簿の承認の欄）
 *
 *   false … 出さない・送らない（何も起きません）
 *   true  … 出す
 *
 * ★**GAS（シフト.gs・コード.gs）と Cloudflare を本番に貼り終えるまで、true にしないでください。**
 *   本番の Cloudflare が申請を知らないうちに提出ページから申請が届くと、「番号のまちがい」と数えられ、
 *   **12回で全員の提出が約10分止まります。**（2026-09-24、ジャーナルの公開と一緒に出すため、止めた形で置きました）
 * ★2026-09-24 夜に true にしました。その前に、本番で次の版を測って確かめてあります：
 *   Cloudflare `e23ef921`（申請の受け口・承認と断る）／GAS コード.gs `d52b7818`・シフト.gs `b09eca64`。
 *   ★止めるときは、ここを false にして公開するだけです（サーバーは貼り直さなくてかまいません）
 */
 ...
 *   組む画面の日付ごとに「ヘルプ要請」（人が欲しい）と「人員過多」（人が余る）を出せて、
 *   キッチンかホールかと人数を選びます。**4店舗どうしで見られます**（「ヘルプ要請・人員過多を見る」。自分の店舗の分も出ます）。
 *
 ...
 *     まぎれないようにしました。
 * ★1項目＝1日×種類×持ち場。キーは `YYYY-MM-DD|help|k`、中身は `{ n: 人数, at, b }`。
 *   `b` はヘルプ要請だけ：出したときに、その日・その持ち場に入っていた人数（2026-09-24 から）。
 *   ここから増えた人数だけ「あと何人」が減り、0 になると消えます（js/app.js の helpReqLeft）。
 ...
 * ★送り元の店舗の「いつ・誰を・どこへ」は、**持たずに、毎回他の店舗の行から
...
const NIPPOU_LAYOUTS = {
  baguru: [
    ...
    // 2026年9月から。行が他の店舗と同じになりました（19〜23行のこぼれも同じ）
    {
      from: 202609,
      cells: {
...
function nippouCheck(o) {
  ...
  //   （開店した月に、原価が売上を上回った店舗が実際にありました。
  //    ★どの店舗の、いつの、いくらかは書きません。このファイルは公開されます。
  //     もとの数は、非公開の会議資料のシートにあります）
