 ...
 *    （ko-dai さんの指示。指示がない限り動かさない）。他のボタンは**この上**に足してください。
...
const ShareView = (() => {
  const 渡すもの = {
    ...
    shift: {
      ...
      絵の名: 'シフト提出ページのQRコード',
      /* ★2026-09-25、ここは「シフトの番号は、1人ずつ別に送ってください」でした。
           番号の申請（店長の承認で番号が入る形）が出たので、**新しい人には当てはまりません**。
         ★**ただし「全部申請になった」でもありません。**現場のマネージの名簿には、いまも
           「送りずみ ◯/◯人」と送りずみのチェックが**出ています**（`manage/js/admin.js`。
           2026-09-25 に現場から取って確かめました）。**番号を作り直したときは、その人に届ける道が要る**ためです
           （`js/app.js` の名簿の所に、シフトがその理由を書いています）。
         ★**だから両方を書きます。**片方だけにすると、作り直したときに誰も届けなくなります。
       ★**2026-09-25 に、もう一度直しました。**はじめの直しは「番号を作り直したとき**だけ**」と書いていました。
         **「だけ」がまちがいです。**手で渡す場面は、現場に**少なくとも3つ**あります。
           ① 番号を作り直したとき（マネージの名簿）
           ② 名簿の枠に**手で足した**とき（番号が自動で入らないので「見る」で出して渡す）
           ③ **同じ名前の人が申請した**とき（「本人なら、名簿の番号を今までどおり LINE で送って、
              この申請は「断る」を押してください」。★ko-dai さんが「この案内は残して」と言った所です）
         ★**数えおわる前に「だけ」と書きました。**同じまちがいをしないよう、ここでは**数えずに**、
           「**渡すときは、その人にだけ**」という**守り方**を書きます。場面が増えても、この文は古くなりません。 */
      別に: '番号がまだ無い人は、開いた画面の「番号をまだ持っていない」から申請できます。'
        + '番号を渡すときは、まとめて送らず、その人にだけ直接お伝えください（アプリの番号とは別のものです）。',
    },
  };
