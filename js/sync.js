...
function この失敗はなにか(e, 上限ms) {
  ...
  if (たぐい === '渡す口') {
    // ★2026-09-19 の切り替えのあと、同期は Cloudflare の口です（ジャーナル・日報の ask は Google のまま）。
    //   どちらでも正しいように「サーバー側」と書きます（前は「Google 側」）
    return 'サーバー側で結果を受け取れませんでした。電波の問題ではありません。'
      + '自動で送り直します（入力は消えません）';
  }
...
const Sync = {
  ...
  _上限: 0,
  /**
   * 「他の端末が動いている」と分かっているあいだの終わり時刻
   *
   * ★他の端末の変更を受け取ったら、しばらく速く取りに行きます。
  ...
  _applyPulled(records, settings) {
    /* ★**本当に中身が変わったもの**を数えます（2026-09-18）。
         ...
         そのあいだに他の端末で直した分（お店で足したシフトの名簿・教育を受ける人など）を**丸ごと上書き**します。
    ...
    if (records.length) {
      ...
      //   他の端末から届いた変更だけ別の場所へ入り、画面に出なくなります
  ...
  legendHtml() {
    const rows = [
      ...
      ['error', '送れていません', '3回送り直しても通らなかったときです。理由は帯に出ます。'
        + '「電波が届いていません」なら待つだけ。「サーバーが返事をしません」「サーバー側で結果を受け取れませんでした」なら'
   ...
   *
   *   他の端末が動いている     … 3秒おき（見ている店舗の記録が届いてから40秒だけ）
   ...
   *   「他の端末がいま入力している」あいだだけ速くする、という配り方です。
