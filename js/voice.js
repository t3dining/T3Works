...
const VoiceView = (() => {
  ...
  /**
   * この端末が管理か … null＝まだ分からない／true／false
   *
   * ★**`isMine()` は「マインのアプリか」で、「管理か」ではありません**（`js/config.js` の `isMine`）。
   *   はじめ `isMine()` だけで返事の欄を出していて、**管理でない社員のマインにも欄が出ていました**。
   *   そこで返事を書くとサーバーが `need_admin` で断り、**書いた返事は消えます**
   *   （2026-09-24、シフトの分野が見つけ、本部から知らせ。送信箱が詰まる方は本部が直しました）。
   * ★**画面で隠すのは守りではありません**（守りはサーバー側で、もう効いています）。
   *   目的は「**できないことを、できるように見せない**」こと。押せる所があると、人は押して、消えて、
   *   理由が分からないままになります。
   * ★聞き方はシフトの名簿（`js/app.js` の `shiftRosterProbe`）と同じ形です。
   *   通信できなかったときは「分からない」のまま＝**勝手に出しません**。
   */
  let 管理か = null;
  let 管理を聞いている = false;

  /** サーバーの答えを入れます（検算からも呼びます） */
  function 管理の答えを入れる(r) {
    管理を聞いている = false;
    if (r && !r.error) 管理か = !!r.admin;
  }

  /** マインのときだけ、サーバーに「管理か」を聞きます（現場のアプリでは聞きません） */
  function 管理かを聞く() {
    if (!(typeof isMine === 'function' && isMine())) return;
    if (管理を聞いている) return;
    // ★PINを入れていない端末では聞きません（聞いても答えが出ません）
    if (typeof Sync === 'undefined' || !Sync.enabled || !Sync.enabled() || !Sync.pin() || !Sync.probeAdmin) return;
    管理を聞いている = true;
    Sync.probeAdmin().then((r) => {
      管理の答えを入れる(r);
      // ★1件の画面を開いているときだけ描き直します（出す画面で描き直すと、打っていた字が消えるため）
      if (いまの画面 === 'one') 出す();
    }).catch(() => { 管理を聞いている = false; });
  }

  /** 返事の欄と、他人の投稿を消すボタンを出すか（★本当の可否はサーバーが確かめます） */
  function 返事を書ける() {
    return typeof isMine === 'function' && isMine() && 管理か === true;
  }

  /** マインで返事の欄を出さないとき、なぜ出ないのかを一行で（現場のアプリでは何も出しません） */
  function 返事の欄が無いわけ() {
    if (!(typeof isMine === 'function' && isMine()) || 管理か === true) return '';
    if (管理か === false) return '返事を書けるのは、管理の番号の人だけです。';
    if (管理を聞いている) return '返事の欄は、管理の番号かどうかを確かめてから出します（確かめています…）';
    return '管理の番号かどうか確かめられないため、返事の欄を出していません。電波の良い所で開き直してください。';
  }
  ...
  function ひとつの絵(印) {
    ...
    const 書く欄 = 返事を書ける()
      ...
      : (返事の欄が無いわけ() ? `<p class="voice-note">${文字(返事の欄が無いわけ())}</p>` : '');
  ...
  function 開く() {
    const panel = document.getElementById('voicePanel');
    if (!panel) return;
    // ★開くたびに聞き直します。番号を入れ替えた端末で、**前の人の答えを使わない**ため
    管理か = null;
    管理かを聞く();
    ヘッダーの下に置く();
    いまの画面 = 'list';
  ...
  return {
    ...
    // ★検算用：返事の欄を出す条件（`isMine()` だけでなく、サーバーの「管理」の答えを見ているか）
