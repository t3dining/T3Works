...
/**
 * サーバー側で落ちたときの文。★電波の話と分けます（アルバイトが自分の電波のせいだと思うため）
 * ★2026-09-19 まで「Google 側で…」でした。13:59 に Cloudflare へ切り替わり、落ちる場所が Google ではなくなったので、
 *   ko-dai さんの了承で「サーバー側」にしました（どちらでも、電波のせいではないと伝わるのは同じです）
 */
const サーバー側の文 = 'サーバー側で受け取れませんでした（電波の問題ではありません）。少し待ってもう一度お試しください';
 ...
 *
 * ★2026-09-19 13:59 から、答えるのは Cloudflare です（cloudflare/worker.js の「シフトの提出」。本部の切り替え M6）。
 *   送り直してよい理由は、そちらでも同じく成り立ちます（1b のときに数えました）：
 *   ・`open` は D1 を読むだけです（番号まちがいの数え `shiftFail` だけは、上と同じ増え方をします）。
 *   ・`put` は「この内容にする」上書き（shiftWishPut）です。半月の行が読んだときのままのときだけ書き、
 *     2本走っても行は1つ・中身も同じです（鍵の中で読み直す代わり）。
 *   ・Worker のシフトの節にも、外への呼び出しはありません。
 *   ★Cloudflare でも、上限（1日の回数）などで JSON でない返事が来ることはあります。それも「渡す口」として送り直します。
 *     時間切れのときは、Worker の中ではまだ動いているかもしれません。**Worker の put が上書きでなくなったら、
 *     返事なしは送り直さないでください。**
 */
async function call(body, opt) {
  ...
  for (let 回 = 0; ; 回 += 1) {
    ...
    if (o.送り直さない || 回 >= 送り直しの間ms.length) {
      ...
      return { ok: false, error: サーバー側の文, たぐい };
    }
...
function applyOpen(res) {
  ...
  // ★他の店舗の分が、この返事に一緒に入っていたら控えます（`res.all`）。
...
function renderBuilt() {
  ...
  shiftDays(shown.y, shown.m, shown.half).forEach((dateStr) => {
    ...
    if (isClosedOn(dateStr)) {
      ...
      // ★この店舗から**ヘルプに出した人**（GAS の shiftHelpOut_ が、他の店舗の
...
/* -------- 出し方 --------
 *
 *  ★普段は出しません。分からなくなった人が「？出し方」を押したときだけ
...
async function boot() {
  ...
  //   「また入れるのか」と思わせてしまいます
  show('boot');
  // ★サーバー側で落ちて送り直すときは、そう書き足します。ぐるぐるだけが長く続くと、
  //   固まったと思って閉じられてしまうためです
  const res = await call({ mode: 'open' }, {
    送り直すとき: () => {
      el('bootText').textContent = '読み込んでいます…（サーバーの返事が遅いので、送り直しています）';
    },
  });
